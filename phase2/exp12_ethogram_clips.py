"""
Experiment 12: Full ethogram clip extraction.

For every event in 'data/Nity events.csv':
  1. Parse date + time (handles "all day" → 10:00, "~HH:MM" → strip ~, etc.)
  2. Find the Right_Top video segment on the server
  3. Download it
  4. Scan ±2.5 min around event time for peak motion (MOG2 with warm-up skip)
  5. Extract 15-second clip at peak
  6. DELETE the raw video immediately to save disk
  7. Record result in data/ethogram_clips.json (saved after every event)

Fully resumable: if clip already exists in data/clips/ethogram/, the event is skipped.
"""
...
CLIPS_DIR  = os.path.join(PROJECT, "data/clips/ethogram")
JSON_OUT   = os.path.join(PROJECT, "data/ethogram_clips.json")
...
SLACK_SEC     = 150    # ±2.5 min scan window around event time
...
ALLDAY_ANCHOR = 10 * 3600   # "all day" → use 10:00
...
def parse_time(t_str):
    ...
    if t.lower() in ("all day", "morning and afternoon"):
        return ALLDAY_ANCHOR, "all_day_anchor_10:00"
    t = t.lstrip("~").strip()
    t = re.sub(r'\s*\(?[\?!]\)?\s*$', '', t).strip()  # strip trailing (?) etc
    if re.search(r'[?!+]', t):
...
def download_video(date, seg, session):
    """Download to TEMP_DIR. Returns local path or None."""
...
def main():
    ...
    for idx, row in enumerate(rows):
        ...
        # Check clip already exists (in case of manual re-run)
        ...
        try:
            ...
            if clip_start is None: clip_start = max(0, offset - 5)
            ...
            if r.returncode == 0 and os.path.exists(clip_path) and os.path.getsize(clip_path) > 10_000:
                ...
                results.append({
                    ...
                    "clip_path": f"data/clips/ethogram/{clip_name}",
            ...
            # Always delete the raw video after processing
