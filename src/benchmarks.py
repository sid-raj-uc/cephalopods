...
def run_seg(ckpt):
    ...
    return {"n_frames": len(pos), "n_negatives": len(neg),
...
def run_skel(ckpt, refine=False):
...
\begin{table}[t]
\caption{Frozen benchmark suite. All splits are by SOURCE VIDEO; the segmentation test videos are
excluded from every training source. Tip-F1 scores arm recovery against the human mask's
protrusions, so both missed arms and spurious ones are penalised.}
...
def to_latex(res):
    ...
    if "seg" in res:
        s = res["seg"]
        rows += [f"SEG-TEST ({s['n_frames']} frames, {s['n_videos']} held-out videos) & mask IoU (mean / median) & "
                 f"{s['iou_mean']:.3f} / {s['iou_median']:.3f} \\\\",
                 f" & body-area error & {s['area_err_pct']:.2f}\\% \\\\"]
        if s.get("presence_auc") is not None:
            rows.append(f" & presence AUC (vs {s['n_negatives']} empty) & {s['presence_auc']:.3f} \\\\")
    if "skel" in res:
        k = res["skel"]
        rows += [r"\midrule",
                 f"SKEL-50 ({k['n_frames']} frames) & arm-tip F1 & {k['tip_f1']:.3f} \\\\",
                 f" & tip precision / recall & {k['tip_precision']:.3f} / {k['tip_recall']:.3f} \\\\",
                 f" & arms per frame (descriptive) & {k['arms_per_frame']:.2f} \\\\"]
        if "head_err_radii_mean" in k:
            rows.append(f" & head error (body radii) & {k['head_err_radii_mean']:.2f} \\\\")
    if "track" in res:
        t = res["track"]
        rows += [r"\midrule",
                 f"TRACK-10 ({t.get('n_clips',0)} clips) & teleport rate & {t.get('teleport_rate',0):.3f} \\\\",
                 f" & occluded fraction & {t.get('occluded_frac',0):.3f} \\\\",
                 f" & coverage & {t.get('coverage',0):.3f} \\\\"]
...
def main():
    ...
    if "seg" in suites:
        print("SEG-TEST …", flush=True); res["seg"] = run_seg(args.ckpt); print(" ", res["seg"], flush=True)
    if "skel" in suites:
        print("SKEL-50 …", flush=True); res["skel"] = run_skel(args.ckpt, args.refine); print(" ", res["skel"], flush=True)
    if "track" in suites:
